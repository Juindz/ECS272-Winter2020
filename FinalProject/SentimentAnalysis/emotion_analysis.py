...
def get_category_score():
    for user in users:
        with open("new_output_"+user+".csv", 'r') as fin, open("output_"+user+".csv",  "w", newline='') as fout,\
                open("new_output_"+user+"_words.csv", "w", newline='') as fout_words:
            reader = csv.reader(fin, delimiter=',')
            writer = csv.writer(fout)
            #writer.writerow(['DateTime', 'Original Tweet', 'Processed Tweet', 'Basic Emotion Vector'])
            words_writer = csv.writer(fout_words)
            words_writer.writerow(['Word', 'Valence', 'Arousal', 'Dominance', 'DateTime', 'Categories'])
            for row in reader:
                if row[0] == 'DateTime':
                    continue
                ...
                for word in words:
                    ...
                    if word in dic_category.keys() and dic_VAD.keys():
                        word_list.append(dic_VAD[word][0])
                        word_list.append(dic_VAD[word][1])
                        word_list.append(dic_VAD[word][2])
                        ...
                        word_list.append(word_categories)
                        words_writer.writerow(word_list)
                    else:
                        ...
                        for synonym in synonyms:
                            if synonym in dic_category.keys() and synonym in dic_VAD.keys():
                                synonyms_num += 1
                                synonyms_v += float(dic_VAD[synonym][0])
                                synonyms_a += float(dic_VAD[synonym][1])
                                synonyms_d += float(dic_VAD[synonym][2])
                                for synonym_category in dic_category[synonym]:
                                    ...
                                    if synonym_category not in word_categories:
                                        word_categories.append(synonym_category)
                        for i in range(0, 8):
                        ...
                        if len(word_categories) > 0:
                            word_list.append(round(synonyms_v / float(synonyms_num), 3))
                            word_list.append(round(synonyms_a / float(synonyms_num), 3))
                            word_list.append(round(synonyms_d / float(synonyms_num), 3))
                            word_list.append(row[0])
                            word_list.append(word_categories)
                            words_writer.writerow(word_list)
